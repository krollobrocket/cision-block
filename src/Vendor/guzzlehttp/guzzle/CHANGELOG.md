...
- `CisionBlock\GuzzleHttp\MessageFormatterInterface`
...
* Improvement: Added `CisionBlock\GuzzleHttp\Exception\InvalidArgumentException` [#2163](https://github.com/guzzle/guzzle/pull/2163)
* Improvement: Added `CisionBlock\GuzzleHttp\_current_time()` to use `hrtime()` if that function exists. [#2242](https://github.com/guzzle/guzzle/pull/2242)
...
* Improvement:  	Use `\CisionBlock\GuzzleHttp\Promise\rejection_for` function instead of object init [#1827](https://github.com/guzzle/guzzle/pull/1827)
...
* Feature: added `CisionBlock\GuzzleHttp\json_encode` and `CisionBlock\GuzzleHttp\json_decode`.
...
* Added `CisionBlock\GuzzleHttp\RequestOptions` to allow request options to be applied
  using constants.
...
* Marked `CisionBlock\GuzzleHttp\Client::getDefaultUserAgent` as deprecated.
...
* Updated to use PSR-7
  ...
  * Removed the dependency on `guzzlehttp/streams`. These stream abstractions
    are available in the `guzzlehttp/psr7` package under the `CisionBlock\GuzzleHttp\Psr7`
    namespace.
* Added middleware and handler system
  * Replaced the Guzzle event and subscriber system with a middleware system.
  * No longer depends on RingPHP, but rather places the HTTP handlers directly
    in Guzzle, operating on PSR-7 messages.
  * Retry logic is now encapsulated in `CisionBlock\GuzzleHttp\Middleware::retry`, which
    means the `guzzlehttp/retry-subscriber` is now obsolete.
  * Mocking responses is now handled using `CisionBlock\GuzzleHttp\Handler\MockHandler`.
* Asynchronous responses
  * No longer supports the `future` request option to send an async request.
    Instead, use one of the `*Async` methods of a client (e.g., `requestAsync`,
    `getAsync`, etc.).
  * Utilizing `CisionBlock\GuzzleHttp\Promise` instead of React's promise library to avoid
...
* Request options
  * POST and form updates
    ...
    * Removed the `CisionBlock\GuzzleHttp\Post` namespace.
...
* Removed `CisionBlock\GuzzleHttp\BatchResults`. Batched pool results are now returned as
  an array.
* Removed `CisionBlock\GuzzleHttp\Query`. Query string handling is now handled using an
  associative array passed into the `query` request option. The query string
  is serialized using PHP's `http_build_query`. If you need more control, you
  can pass the query string in as a string.
* `CisionBlock\GuzzleHttp\QueryParser` has been replaced with the
  `CisionBlock\GuzzleHttp\Psr7\parse_query`.
...
* Added a `CisionBlock\GuzzleHttp\Pool::send()` convenience method to match the docs.
...
* Added a "progress" event so that you can get upload and download progress
  events.
* Added `CisionBlock\GuzzleHttp\Pool` which implements FutureInterface and transfers
  requests concurrently using a capped pool size as efficiently as possible.
* Added `hasListeners()` to EmitterInterface.
* Removed `CisionBlock\GuzzleHttp\ClientInterface::sendAll` and marked
  `CisionBlock\GuzzleHttp\Client::sendAll` as deprecated (it's still there, just not the
  recommended way).
...
* Removed the fluent interfaces (i.e., `return $this`) from requests,
  responses, `CisionBlock\GuzzleHttp\Collection`, `CisionBlock\GuzzleHttp\Url`,
  `CisionBlock\GuzzleHttp\Query`, `CisionBlock\GuzzleHttp\Post\PostBody`, and
  `CisionBlock\GuzzleHttp\Cookie\SetCookie`. This blog post provides a good outline of
  why I did this: https://ocramius.github.io/blog/fluent-interfaces-are-evil/.
  This also makes the Guzzle message interfaces compatible with the current
  PSR-7 message proposal.
* Removed "functions.php", so that Guzzle is truly PSR-4 compliant. Except
  for the HTTP request functions from function.php, these functions are now
  implemented in `CisionBlock\GuzzleHttp\Utils` using camelCase. `CisionBlock\GuzzleHttp\json_decode`
  moved to `CisionBlock\GuzzleHttp\Utils::jsonDecode`. `CisionBlock\GuzzleHttp\get_path` moved to
  `CisionBlock\GuzzleHttp\Utils::getPath`. `CisionBlock\GuzzleHttp\set_path` moved to
  `CisionBlock\GuzzleHttp\Utils::setPath`. `CisionBlock\GuzzleHttp\batch` should now be
  `CisionBlock\GuzzleHttp\Pool::batch`, which returns an `objectStorage`. Using functions.php
  caused problems for many users: they aren't PSR-4 compliant, require an
  explicit include, and needed an if-guard to ensure that the functions are not
  declared multiple times.
* Rewrote adapter layer.
    * Removing all classes from `CisionBlock\GuzzleHttp\Adapter`, these are now
      implemented as callables that are stored in `CisionBlock\GuzzleHttp\Ring\Client`.
    * Removed the concept of "parallel adapters". Sending requests serially or
      concurrently is now handled using a single adapter.
    * Moved `CisionBlock\GuzzleHttp\Adapter\Transaction` to `CisionBlock\GuzzleHttp\Transaction`. The
...
* `CisionBlock\GuzzleHttp\Message\MessageFactory` no longer allows subclasses to add
...
* Updated to "CisionBlock\GuzzleHttp\Streams" 3.0.
    * `CisionBlock\GuzzleHttp\Stream\StreamInterface::getContents()` no longer accepts a
      `maxLen` parameter. This update makes the Guzzle streams project
      compatible with the current PSR-7 proposal.
    * `CisionBlock\GuzzleHttp\Stream\Stream::__construct`,
      `CisionBlock\GuzzleHttp\Stream\Stream::factory`, and
      `CisionBlock\GuzzleHttp\Stream\Utils::create` no longer accept a size in the second
...
* Added `ToArrayInterface` to `CisionBlock\GuzzleHttp\Cookie\CookieJar`

## 4.1.0 - 2014-05-27

* Added a `json` request option to easily serialize JSON payloads.
* Added a `CisionBlock\GuzzleHttp\json_decode()` wrapper to safely parse JSON.
* Added `setPort()` and `getPort()` to `CisionBlock\GuzzleHttp\Message\RequestInterface`.
...
* Added `CisionBlock\GuzzleHttp\batch()` as a convenience function for sending requests in
  parallel without needing to write asynchronous code.
* Restructured how events are added to `CisionBlock\GuzzleHttp\ClientInterface::sendAll()`.
